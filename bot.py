...
def generate_ideas(topic):
    prompt = (
        ...
        f"Каждая идея — одно короткое предложение с призывом к действию, минимум 5 слов, обязательно с глаголом, без лишнего текста, вроде 'Вот три идеи...', и без нумерации. "
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            filtered_ideas = [idea for idea in ideas if len(idea.split()) >= 5 and any(word in idea for word in ["покажи", "сними", "создай", "поделись", "запусти", "сфотографируй", "расскажи"])]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if re.search(r'[^\u0400-\u04FF\s\d.,!?():;-]' if lang == "ru" else r'[^\x00-\x7F\s\d.,!?():;-]', corrected_text):
                    logger.warning("Обнаружены не-{lang} символы, заменяю...")
                    replacements = {
                        'aged': 'в возрасте', 'thoughts': 'мысли', 'confidence': 'уверенность', 'find': 'найти',
                        'shares': 'поделись', 'healthy': 'здоровый', 'lifestyle': 'образ жизни'
                    } if lang == "ru" else {}

...
def generate_ideas(topic, style="саркастичный"):
    prompt = (
        ...
        f"СТРОГО ТОЛЬКО 3 ИДЕИ, НИ В КОЕМ СЛУЧАЕ НЕ ПИШИ ВВОДНЫЕ ФРАЗЫ вроде 'Ты получил три уникальные идеи', 'Вот три идеи', 'Here are three unique ideas', 'Следующие идеи' или любые другие пояснения, только сами идеи, по одной на строку, без нумерации или лишнего текста. "
        ...
        f"Примеры для темы 'кофе' в стиле 'формальный': "
        f"Организуйте дегустацию кофе и определите лучший сорт для рабочего дня "
        f"Подготовьте обзор методов заваривания кофе и поделитесь результатами с коллегами "
        f"Проведите анализ влияния кофе на продуктивность и опубликуйте выводы "
    ...
    if user_id in user_data and "mode" in user_data[user_id] and "stage" in user_data[user_id]:
        ...
        if mode == "name" and stage == "ask_name":
            ...
            if message.isdigit() and 1 <= int(message) <= 3:
                ...
                del user_data[user_id]  # Полная очистка после генерации
            else:
            ...
            del user_data[user_id]  # Полная очистка после редактирования

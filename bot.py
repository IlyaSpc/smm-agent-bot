...
def generate_ideas(topic, style="саркастичный"):
    prompt = (
        ...
        f"Каждая идея — одно полное предложение с призывом к действию и глаголом, минимум 5 слов. "
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            filtered_ideas = [idea for idea in ideas if len(idea.split()) >= 5 and not re.search(r'[^\u0400-\u04FF\s\d.,!?():;-]', idea)]
            if len(filtered_ideas) < 3:
                fallback_ideas = {
                    "саркастичный": [
                        f"Сними свой провал на видео и убеди всех, что ты мастер {topic}",
                        f"Похвастайся своими ошибками в сторис и собери лайки от сочувствующих",
                        f"Запусти челлендж 'Докажи, что хуже некуда' и вдохнови всех бросить затею"
                    ],
                    "дружелюбный": [
                        f"Сделай уютное фото с {topic} и расскажи друзьям, как это поднимает настроение",
                        f"Пригласи подписчиков на виртуальную встречу и поделись своим опытом с {topic}",
                        f"Покажи, как {topic} помогает тебе, и предложи всем попробовать это повторить"
                    ],
                    "формальный": [
                        f"Организуйте тематический вечер с {topic} и зафиксируйте впечатления для коллег",
                        f"Проведите анализ преимуществ {topic} и опубликуйте результаты для аудитории",
                        f"Подготовьте обзор лучших практик с {topic} и поделитесь выводами с подписчиками"
                    ]
                }
...
def generate_hashtags(topic):
    ...
    thematic_hashtags = {
        ...
        "зима": ["#зима", "#снег", "#холод", "#отдых", "#природа", "#уют"],
        "утро": ["#утро", "#энергия", "#день", "#мотивация", "#спокойствие", "#начало"],
        "день_ночь": ["#день", "#ночь", "#жизнь", "#энергия", "#спокойствие", "#мотивация"],
        ...
        "посты_про_хлеб": ["#хлеб", "#выпечка", "#вкус", "#утро", "#еда", "#свежесть", "#традиции", "#пекарня"]
    }
    relevant_tags = []

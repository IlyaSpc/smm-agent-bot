...
def generate_ideas(topic):
    prompt = (
        f"Ты креативный SMM-специалист. Придумай ровно 3 уникальные идеи для постов или стおり на тему '{topic}' "
        ...
        f"Похвастайся самым нелепым знакомством и удиви всех "
        f"Сфоткай свою визитку в странном месте и собери лайки "
        f"Запусти марафон '5 рукопожатий за неделю' и докажи, что ты профи"
    )
    ...
    if mode in ["post", "story"]:
        ...
        if mode == "post":
            full_prompt = (
                ...
                f"Стиль: {style}, саркастичный — язвительный, с чёрным юмором, но без оскорблений, краткий, ясный, с фактами; дружелюбный — тёплый, с лёгким юмором; формальный — чёткий, профессиональный. "
            ...
            full_prompt = (
                ...
                f"Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
                f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                f"Стиль: {style}, саркастичный — язвительный, с чёрным юмором, но без оскорблений; дружелюбный — тёплый, с лёгким юмором; формальный — чёткий, профессиональный. "
                f"Пиши ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, без иностранных слов. "
            ...
            if response.status_code == 200:
                ...
                if re.search(r'[^\u0400-\u04FF\s\d.,!?():;-]', corrected_text):
                    ...
                    replacements = {
                        ...
                        'mindfulness': 'осознанность', 'mental': 'психическое', 'health': 'здоровье', 'shares': 'поделись'
                    }
...
def generate_hashtags(topic):
    ...
    return " ".join(combined).replace(" #", "#")  # Убираем лишние пробелы

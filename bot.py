...
def generate_ideas(topic, style="саркастичный", user_id=None):
    ...
    max_attempts = 3
    filtered_ideas = []
    forbidden_phrases = ["ты получил", "вот три", "здесь твои", "идеи для", "following ideas", "саркастичный стиль", "дружелюбный стиль", "формальный стиль"]
    ...
    if len(filtered_ideas) < 3:
        ...
        fallback_ideas = {
            "саркастичный": [
                ...
                f"Докажи, что {topic} не для слабаков, и поделись своим провалом с нами!",
                f"Освой {topic} или пинай себя за лень до конца своих дней!"
            ],
            "дружелюбный": [
                f"Попробуй {topic} сегодня и расскажи, как весело ты провёл время!",
                f"Докажи, что {topic} — это круто, и поделись своими эмоциями с нами!",
                f"Освой {topic} с нами и покажи всем, как легко это получается!"
            ],
            "формальный": [
                f"Изучите {topic} и представьте свои результаты в комментариях ниже.",
                f"Начните применять {topic} и продемонстрируйте достигнутый успех.",
                f"Освойте {topic} и сообщите о своих достижениях прямо сейчас."
            ]
        }
        ...
        if mode in ["post", "story"]:
            ...
            if mode == "post":
                full_prompt = (
                    ...
                    f"Напиши пост на русском языке (10-12 предложений) по теме '{topic.replace('_', ' ')}' для социальных сетей. "
                    f"Обязательно используй идею: '{idea}' — текст должен строго следовать этой идее, развивать её и не отклоняться в сторону. "
                    ...
                    f"Пиши ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, без иностранных слов, избегай повторов, обрывов и мусорных фраз вроде 'инг', 'он .', 'это'. "
                ...
                if template == "опрос":
                    full_prompt = (
                        f"Ты копирайтер с 10-летним опытом, работающий на основе книг 'Пиши, сокращай', 'Клиентогенерация' и 'Тексты, которым верят'. "
                        f"Напиши сторис на русском языке (3-5 предложений) по теме '{topic.replace('_', ' ')}' для социальных сетей. "
                        f"Обязательно используй идею: '{idea}' — текст должен строго следовать этой идее, развивать её и не отклоняться в сторону. "
                        f"Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
                        f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                        f"Стиль: {style}, саркастичный — язвительный, с чёрным юмором, без оскорблений; дружелюбный — тёплый, с лёгким юмором; формальный — чёткий, профессиональный. "
                        f"Шаблон: опрос — задай вопрос с 2-3 вариантами ответа и предложи подписчикам выбрать один. "
                        f"Пиши ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, без иностранных слов, избегай повторов текста, убедись, что текст логичен, без обрывов и мусорных фраз вроде 'Он .' или 'инг'. "
                    ...
                    full_prompt = (
                        f"Ты копирайтер с 10-летним опытом, работающий на основе книг 'Пиши, сокращай', 'Клиентогенерация' и 'Тексты, которым верят'. "
                        f"Напиши сторис на русском языке (6-8 предложений) по теме '{topic.replace('_', ' ')}' для социальных сетей. "
                        f"Обязательно используй идею: '{idea}' — текст должен строго следовать этой идее, развивать её и не отклоняться в сторону. "
                        f"Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
                        f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                        f"Стиль: {style}, саркастичный — язвительный, с чёрным юмором, без оскорблений; дружелюбный — тёплый, с лёгким юмором; формальный — чёткий, профессиональный. "
                    ...
                    full_prompt = (
                        f"Ты копирайтер с 10-летним опытом, работающий на основе книг 'Пиши, сокращай', 'Клиентогенерация' и 'Тексты, которым верят'. "
                        f"Напиши сторителлинг на русском языке (6-8 предложений) по теме '{topic.replace('_', ' ')}' для социальных сетей. "
                        f"Обязательно используй идею: '{idea}' — текст должен строго следовать этой идее, развивать её и не отклоняться в сторону. "
                        f"Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
                        f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                        f"Стиль: {style}, саркастичный — язвительный, с чёрным юмором, без оскорблений; дружелюбный — тёплый, с лёгким юмором; формальный — чёткий, профессиональный. "
            ...
            full_prompt = (
                ...
                f"2) Перечисли 5-7 уникальных болей аудитории (нумерованный список, каждый пункт — полное предложение, без повторов). "
                f"3) Перечисли 5-7 уникальных желаний аудитории (нумерованный список, каждый пункт — полное предложение, без повторов). "
                f"4) Опиши момент покупки: эмоции, желания, барьеры, детали по теме '{topic}' (минимум 3 предложения). "
                f"5) Создай 5 уникальных персонажей ЦА: имя, возраст, профессия, цели, боли, занятия, цитата (например, 'Я хочу...') — каждый персонаж в отдельном абзаце, планы действий должны отличаться. "
                f"6) Для каждого персонажа — план действий (3-5 шагов) с разными форматами (посты, видео, сторис) и соцсетями ({channels}) — опиши конкретно и уникально, что публиковать для решения их болей или желаний. "
                f"7) Создай общий план привлечения: 5-7 конкретных шагов с описанием типов контента (посты, видео, сторис) для {channels}, направленных на достижение '{result}'. "
                f"8) Заверши отдельным призывом к действию (1-2 предложения, начинай с глагола, например, 'Присоединяйтесь...') и метриками (включая {result}, конверсию и доход — например, '1000 лидов, 20% конверсия, 50000 рублей дохода') — без дублирования метрик. "
            ...
            full_prompt = (
                ...
                f"Обязательно основывайся на стратегии: '{strategy_text[:1000]}' — используй её боли, желания и персонажей для создания идей. "
                f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                f"Пиши ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, без иностранных слов. "
                f"Составь план на {duration_weeks} недели: 1) Дата и время публикации. 2) Тип контента (пост, короткое видео). "
...
async def handle_message(update: Update, context: ContextTypes, is_voice=False):
    ...
    if user_id in user_data and "mode" in user_data[user_id] and "stage" in user_data[user_id]:
        ...
        if mode == "name" and stage == "ask_name":
            ...
            if "раза в неделю" not in message.lower():
                await update.message.reply_text(f"{user_names.get(user_id, 'Друг')}, уточни частоту полностью (например, '2 раза в неделю') ⏰")
                return

...
def generate_text(user_id: int, mode: str):
    ...
    if mode == "post":
        full_prompt = (
            ...
            "Напиши пост на русском языке (8-10 предложений) по теме '{topic}' для социальных сетей. "
            "Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
            "Контекст из книг: '{context}'. "
            "Пиши ТОЛЬКО на русском языке, любые иностранные слова категорически запрещены — используй русские эквиваленты (например, 'firsthand' — 'на собственном опыте', 'help us grow' — 'помогают расти', 'family dinner' — 'семейный ужин', 'like' — 'например', 'correct' — 'правильный'). "
            "Стиль: дружелюбный, живой, разговорный, с эмоциями, краткий, ясный, без штампов, оценок и лишних слов, с фактами вместо общих фраз, добавь позитив и лёгкий юмор (если уместно). "
            "Структура: начни с цепляющего вопроса или факта (AIDA), раскрой проблему аудитории, предложи решение, закрой возражения (например, 'а если нет времени?'), покажи выгоду через пример, заверши призывом к действию. Пиши только текст поста."
        ...
        full_prompt = (
            ...
            "Напиши сторителлинг на русском языке (4-6 предложений) по теме '{topic}' для социальных сетей. "
            "Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
            "Контекст из книг: '{context}'. "
            ...
            "Стиль: живой, эмоциональный, с метафорами, краткий, ясный, разговорный, без штампов, с позитивом и лёгким юмором (если уместно). "
        ...
        full_prompt = (
            ...
            "Разработай стратегию клиентогенерации на русском языке (10-15 предложений) по теме '{topic}'. "
            "Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
            "Контекст из книг: '{context}'. "
        ...
        full_prompt = (
            "Ты копирайтер с 10-летним опытом, работающий только на основе книг 'Пиши, сокращай', 'Клиентогенерация' и 'Тексты, которым верят'. "
            "Напиши описание изображения на русском языке (3-5 предложений) по теме '{topic}' для социальных сетей. "
            "Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
            "Контекст из книг: '{context}'. "
...
def generate_hashtags(topic):
    ...
    thematic_hashtags = {
        ...
        "собаки": ["#питомцы", "#собачьяжизнь", "#другчеловека", "#животные"],
        "чай": ["#чаепитие", "#утро", "#напитки", "#релакс"],
        "семья": ["#семейныеценности", "#тепло", "#вместе", "#любовь"],
    }
    relevant_tags = []

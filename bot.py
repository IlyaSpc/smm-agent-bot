...
def generate_text(user_id, mode):
    ...
    if mode == "post":
        full_prompt = (
            ...
            "Пиши ТОЛЬКО на русском языке, любые иностранные слова запрещены — используй русские эквиваленты (например, 'firsthand' — 'из первых рук', 'like' — 'например'). "
        ...
        full_prompt = (
            ...
            "Цель текста: {goal}. Главная мысль: {main_idea}. Факты: {facts}. Боли и потребности аудитории: {pains}. "
            "Контекст из книг: '{context}'. "
            "Пиши ТОЛЬКО на русском языке, любые иностранные слова запрещены — используй русские эквиваленты (например, 'firsthand' — 'из первых рук'). "
        ...
        full_prompt = (
            ...
            "Контекст из книг: '{context}'. "
            "Пиши ТОЛЬКО на русском языке, категорически запрещено использовать любые иностранные слова (английские, испанские, немецкие и т.д.) — заменяй их русскими эквивалентами (например, 'firsthand' — 'из первых рук', 'thoughts' — 'мысли', 'regular' — 'регулярный', 'grooming' — 'уход', 'tips' — 'советы', 'satisfied' — 'довольные'). "
            ...
            "4) Опиши момент покупки: эмоции (например, отчаяние, надежда), желания (например, найти решение), барьеры (например, страх потратить деньги впустую, недоверие к специалистам), добавь больше деталей и эмоций, связанных с услугами психолога. "
            ...
            "6) После каждого персонажа добавь подзаголовок 'Как делать!' и предложи уникальный план действий, как мне, как SMM-специалисту, превратить эту персону в клиента бизнеса. Какие способы взаимодействия я могу использовать? Как продавать через соцсети? Распиши по шагам (3-5 шагов) действия, чтобы продать сегменту этой персоны услуги психолога, учитывая её профессию и боли. Планы должны отличаться для каждой персоны, используй разные форматы (посты, короткие видео, сторис) и соцсети (Instagram, ВКонтакте, Telegram), избегая повторений. "
        ...
        full_prompt = (
            ...
            "Контекст из книг: '{context}'. "
            "Пиши ТОЛЬКО на русском языке, категорически запрещено использовать любые иностранные слова — заменяй их русскими эквивалентами (например, 'post' — 'пост', 'reels' — 'короткие видео', 'schedule' — 'график', 'tips' — 'советы', 'satisfied' — 'довольные'). "
            ...
            "Учитывай частоту публикаций, указанную пользователем (например, '2 поста и 3 видео в неделю' значит 4 поста и 6 видео за 2 недели), распредели контент равномерно, используй только посты и короткие видео. Пиши только текст контент-плана."
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if re.search(r'[^\u0400-\u04FF\s\d.,!?():;-]', corrected_text):  # Проверка на не-русские символы
                    ...
                    replacements = {
                        'thoughts': 'мысли', 'regular': 'регулярный', 'grooming': 'уход', 'tips': 'советы', 
...
async def start(update: Update, context: ContextTypes):
    ...
    await update.message.reply_text(
        ...
        "Примеры запросов: 'пост про кофе', 'стори для города', 'стратегия для психолога'.\n"

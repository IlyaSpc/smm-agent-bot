...
def generate_ideas(topic, style="саркастичный"):
    prompt = (
        ...
        f"Каждая идея — одно полное предложение с призывом к действию и глаголом, минимум 5 слов, строго связана с '{topic}', без ухода в другие темы вроде утра или зарядки. "
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(filtered_ideas) < 3:
                ...
                return generate_ideas(topic, style)  # Повторяем, если идей мало
...
def generate_text(user_id, mode):
    ...
    if lang == "ru":
        if mode in ["post", "story"]:
            ...
            full_prompt = (
                ...
                f"ОБЯЗАТЕЛЬНО выполни ВСЕ следующие задачи ПОЛНОСТЬЮ, не пропускай ни один пункт: "
                f"1) Опиши аудиторию: возраст, пол, профессия, интересы, поведение, привычки (минимум 3 предложения). "
                f"2) Перечисли 5-7 болей аудитории (нумерованный список, каждый пункт — полное предложение). "
                f"3) Перечисли 5-7 желаний аудитории (нумерованный список, каждый пункт — полное предложение). "
                f"4) Опиши момент покупки: эмоции, желания, барьеры, детали по теме '{topic}' (минимум 3 предложения). "
                f"5) Создай 5 персонажей ЦА: имя, возраст, профессия, цели, боли, занятия, цитата (например, 'Я хочу...') — каждый персонаж в отдельном абзаце. "
                f"6) Для каждого персонажа — план действий (3-5 шагов) с разными форматами (посты, видео, сторис) и соцсетями ({channels}) — опиши конкретно, что публиковать для решения их болей или желаний. "
                f"7) Создай общий план привлечения: 5-7 шагов с описанием типов контента (посты, видео, сторис) для {channels}, направленных на достижение '{result}'. "
                f"8) Заверши отдельным призывом к действию (1-2 предложения, начинай с глагола, например, 'Присоединяйтесь...') и метриками (включая {result}, конверсию и доход — например, '1000 лидов, 20% конверсия, 50000 рублей дохода'). "
                f"Пиши только текст стратегии, без лишних комментариев и вводных фраз."
            )
            ...
            full_prompt = (
                ...
                f"Основывайся на стратегии: '{strategy_text[:1000]}' — используй её боли, желания и персонажей для идей. "
                f"Контекст из книг: '{BOOK_CONTEXT[:1000]}'. "
                f"Пиши ТОЛЬКО НА РУССКОМ ЯЗЫКЕ, без иностранных слов. "
                f"Составь план на 2 недели: 1) Дата и время публикации. 2) Тип контента (пост, короткое видео). "
                f"3) Краткое описание (2-3 предложения) с идеей, связанной с '{topic}' и стратегией. 4) Цель (привлечение, прогрев, продажа). "
    ...
    payload = {
        ...
        "max_tokens": 5000 if mode == "strategy" else 2000 if mode in ["post", "story"] else 3000,
        "temperature": 0.5
    }
    timeout = 60

...
def generate_text(user_id, mode):
    ...
    if mode == "post":
        ...
        full_prompt = (
            ...
            "Контекст из книг: '{context}'. "
            "Пиши исключительно на русском языке, категорически запрещено использовать любые иностранные слова (английские, испанские, немецкие и т.д.) — заменяй их русскими эквивалентами (например, 'aged' — 'в возрасте', 'thoughts' — 'мысли', 'confidence' — 'уверенность', 'find' — 'найти', 'hearts' — 'сердце'). "
        ...
        if user_data[user_id]["mode"] == "strategy" or user_data[user_id]["mode"] == "content_plan":
            if user_data[user_id]["stage"] == "client":
                ...
                try:
                    ...
                    await asyncio.sleep(15)  # Увеличено до 15 секунд
...
async def start(update: Update, context: ContextTypes):
    ...
    await update.message.reply_text(
        ...
        "Задержка ответа — от 5 до 15 секунд, пока я думаю над твоим запросом. 😊"
    )

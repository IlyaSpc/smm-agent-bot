...
def generate_text(user_id: int, mode: str):
    ...
    if mode == "post":
        full_prompt = (
            ...
            "Пиши ТОЛЬКО на русском языке, любые иностранные слова категорически запрещены — используй русские эквиваленты (например, 'firsthand' — 'на собственном опыте', 'help us grow' — 'помогают расти', 'family dinner' — 'семейный ужин', 'like' — 'например', 'correct' — 'правильный', 'deserves' — 'заслуживает'). "
        ...
        full_prompt = (
            ...
            "Структура: 1) Кто идеальный клиент (отрасль, боли, потребности), 2) Воронка продаж (привлечение через каналы, прогрев контентом, закрытие сделки), 3) Инструменты автоматизации (CRM, рассылки, чат-боты), 4) Метрики успеха (KPI), 5) Призыв к действию. Пиши только текст стратегии."
